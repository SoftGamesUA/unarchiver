...
+ (void)initialize {
    ...
    if (!s_customValuesDic) {
        s_customValuesDic = [[NSMutableDictionary dictionaryWithCapacity:5] retain];
...
+ (NSString *)backgroundImageShadowInsetsKey {
    // use class name to make user defaults key for bg image shadow insets
    return [NSString stringWithFormat:@"%@::BackgroundImageShadowInsets", NSStringFromClass(self.class)];    
}

+ (void)setBackgroundImageShadowInsets:(UIEdgeInsets)shadowInsets {
    // save bg image shadow insets to user defaults
    [self.class setCustomValue:[NSValue valueWithUIEdgeInsets:shadowInsets]  forKey:self.class.backgroundImageShadowInsetsKey];
}

+ (UIEdgeInsets)backgroundImageShadowInsets {
    return [[self.class customValueForKey:self.class.backgroundImageShadowInsetsKey] UIEdgeInsetsValue];
}
...
- (void)configure {
    ...
    UIImage *bgImage = [[UIImage imageNamed:self.class.backgroundImageName]
                        stretchableImageWithLeftCapWidth:bgCapInsets.left topCapHeight:bgCapInsets.bottom];
...
- (void)layoutSubviews {
    ...
    UIEdgeInsets bgShadowInsets = [self.class backgroundImageShadowInsets];
    switch (_arrowDirection) {
        ...
        case UIPopoverArrowDirectionDown:
            ...
            arrowRect = _arrowImageView.frame;  // get frame after (!) transformation
            arrowRect.origin.x = self.bounds.size.width / 2 + _arrowOffset - arrowRect.size.width / 2;            
            arrowRect.origin.y = self.bounds.size.height - arrowRect.size.height - bgShadowInsets.bottom;
            // popovers are usually not scaled down by x, so no need to account for that now            
            break;
        case UIPopoverArrowDirectionLeft:
            ...
            arrowRect.size.width += bgShadowInsets.left;
        ...
        case UIPopoverArrowDirectionRight:
            ...
            // get the frame (not bounds! because bounds ignore transformations)
            arrowRect = _arrowImageView.frame;  // get frame after (!) transformation
            arrowRect.origin.x = self.bounds.size.width - arrowRect.size.width - bgShadowInsets.right;      
            arrowRect.origin.y = self.bounds.size.height / 2 + _arrowOffset - arrowRect.size.height / 2;   
            // final adjustment - check that the arrow is not left below popover (happens when keyboard is shown)            
            arrowRect.origin.y = fminf(self.bounds.size.height - arrowRect.size.height  - contentViewInsets.bottom, arrowRect.origin.y);
            ...
            arrowRect.size.width += bgShadowInsets.right;

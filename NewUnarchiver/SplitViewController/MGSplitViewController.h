...
#define CHANGE_SPLIT_POSITION_NOTIFICATION @"change split position"
#define MG_MIN_VIEW_WIDTH				300.0	// minimum width a view is allowed to become as a result of changing the splitPosition.
...
typedef enum _MGSplitViewDividerStyle {
    ...
    MGSplitViewDividerStyleUnarchiver	= 2  // For New Unarchiver.
...
@interface MGSplitViewController : UIViewController <UIPopoverControllerDelegate> {
    ...
    UIImageView *_dividerBtn;
    int _minViewWidth;
    NSMutableArray * _autoresizingMasks;
    float _previousSplitPosition;
}

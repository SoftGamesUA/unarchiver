...
#define MG_UNARCHIVERSPLITTER_SPLIT_WIDTH		5.0	// width of split-gutter for MGSplitViewDividerStylePaneSplitter style.

#define CLOSE_VIEW_WIDTH                50
...
@synthesize isDetailHidden = _isDetailHidden;
...
- (void)setup
{
    ...
    _dividerBtn = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"DividerBtn.png"]];
    _dividerBtn.userInteractionEnabled = true;
    [self.view addSubview:_dividerBtn];
    
    UIPanGestureRecognizer *panGesture = [[UIPanGestureRecognizer alloc]	initWithTarget:self action:@selector(handlePanGesture:)];
    [_dividerBtn addGestureRecognizer:panGesture];
    [panGesture release];
    
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc]	initWithTarget:self action:@selector(handleTapGesture:)];
    tapGesture.numberOfTapsRequired = 2;
    [_dividerBtn addGestureRecognizer:tapGesture];
    [tapGesture release];
    
    _minViewWidth = MG_MIN_VIEW_WIDTH;
    _isDetailHidden = false;
    _autoresizingMasks = [[NSMutableArray alloc] init];
}
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    if(_isDetailHidden)
    {
        ...
        _isDetailHidden = true;
        
    } else [self setSplitPosition:self.view.frame.size.width / 2 animated:true];
...
{    
    if(_isDetailHidden)  [self closeDetailVC];
...
- (void)layoutSubviewsForInterfaceOrientation:(UIInterfaceOrientation)theOrientation withAnimation:(BOOL)animate
{
	...
	if ([self isVertical]) {
        ...
        if (_dividerStyle == MGSplitViewDividerStyleUnarchiver) 
        {
            theView = _dividerBtn;
            CGRect dividerBtnRect = theView.frame;
            dividerBtnRect.origin = 
            CGPointMake(dividerRect.origin.x + dividerRect.size.width/2 - dividerBtnRect.size.width/2, 
                                            dividerRect.size.height - dividerBtnRect.size.height/2);
            theView.frame = dividerBtnRect;
            if (!theView.superview) [self.view addSubview:theView];
    ...
    if (_dividerStyle == MGSplitViewDividerStyleUnarchiver) 
    {
        [self.view bringSubviewToFront:_dividerBtn];
    }
...
- (void)setSplitPosition:(float)posn
{
	...
	if (_delegate && [_delegate respondsToSelector:@selector(splitViewController:constrainSplitPosition:splitViewSize:)]) {
		...
		float maxPos = ((_vertical) ? fullSize.width : fullSize.height) - (_minViewWidth + _splitWidth);
		constrained = (newPosn != _splitPosition && newPosn >= _minViewWidth && newPosn <= maxPos);
	}
	...
	if (constrained) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:CHANGE_SPLIT_POSITION_NOTIFICATION object:self];
	}
}
...
- (void)setDividerStyle:(MGSplitViewDividerStyle)newStyle
{
	...
	} else if (_dividerStyle == MGSplitViewDividerStyleUnarchiver) {
		cornerRadius = MG_DEFAULT_CORNER_RADIUS;
		_splitWidth = MG_UNARCHIVERSPLITTER_SPLIT_WIDTH;
		self.allowsDraggingDivider = NO;
	}
...
{
    if (_dividerStyle != MGSplitViewDividerStyleUnarchiver)  return;
    
	UITouch *touch = [touches anyObject];
    CGPoint pt = [touch locationInView:self.view];
	if (CGRectContainsPoint(_dividerBtn.frame, pt)) 
    {
		CGPoint lastPt = [touch previousLocationInView:self.view];
		float offset = (self.vertical) ? pt.x - lastPt.x : pt.y - lastPt.y;
		if (!self.masterBeforeDetail) {
			offset = -offset;
		}
		self.splitPosition = self.splitPosition + offset;
	}
}*/
...
- (void) handleTapGesture:(UITapGestureRecognizer *)sender
{
	...
	{
        if (self.splitPosition == self.view.bounds.size.width - CLOSE_VIEW_WIDTH)
        {
            [self restoreDetailVC];
        }
        else
...
- (void) restoreDetailVC
{
    _isDetailHidden = false;
    _minViewWidth = MG_MIN_VIEW_WIDTH;
    [self setSplitPosition: _previousSplitPosition animated:true];
    
    UINavigationController * navVC = (UINavigationController *)self.detailViewController;
    ...
    for (int i = 0; i < [views count]; i ++)
    {
        ...
        UIViewAutoresizing resizing = [[_autoresizingMasks objectAtIndex:i] intValue];
...
- (void) closeDetailVC
{
    ...
    UIViewController * vc = navVC.visibleViewController;
    NSArray * views = [vc.view subviews];
    [_autoresizingMasks removeAllObjects];
    
    for (int i = 0; i < [views count]; i ++)
    {
        UIView * view = [views objectAtIndex:i];
        NSNumber * resizing = [NSNumber numberWithInt:view.autoresizingMask];
        [_autoresizingMasks addObject:resizing];
        view.autoresizingMask ^= UIViewAutoresizingFlexibleWidth;
    }
    
    _previousSplitPosition = _splitPosition;
    _minViewWidth = CLOSE_VIEW_WIDTH - _splitWidth;
    [self setSplitPosition: self.view.bounds.size.width - CLOSE_VIEW_WIDTH animated:true];
    _isDetailHidden = true;
}
